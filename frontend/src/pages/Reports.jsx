  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-3 mb-6">
        <Select value={selectedId} onChange={(e) => setSelectedId(e.target.value)} options={exercises.map((e) => ({ value: e.id, label: e.name }))} placeholder="Select an exercise…" className="w-72" />
        {selectedId && (
      ...
      {!selectedId && !loadingExercises && <p className="text-reddit-muted text-sm">Select an exercise above to view its report.</p>}
      {selectedId && loadingSummary && <Spinner />}

      {selectedId && summary && (
        <>
          <div className="flex items-center gap-3 mb-5">
            <h2 className="text-lg font-bold text-reddit-text">{selectedExercise?.name}</h2>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
            ...
            <div className="bg-reddit-card border border-reddit-border rounded-lg p-4">
              <p className="text-xs text-reddit-muted font-semibold uppercase tracking-wider">Avg Time to Detect</p>
              <p className="text-2xl font-bold text-reddit-text">
          ...
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
            <div className="bg-reddit-card border border-reddit-border rounded-lg p-5">
              <h3 className="text-sm font-semibold text-reddit-muted uppercase tracking-wider mb-4">Detection Rate by Tactic</h3>
              <DetectionRateChart tacticBreakdown={summary.tactic_breakdown} />
            </div>
            <div>
              <h3 className="text-sm font-semibold text-reddit-muted uppercase tracking-wider mb-4">Tactic Coverage</h3>
